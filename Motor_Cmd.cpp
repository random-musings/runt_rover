...
{
	int leftSide = BACKWARD;
	int rightSide = FORWARD;
	int leftSpeed = FAST;
	int rightSpeed =FAST;
	...
	int iterations = convertDegrees(abs(degrees));
	
 for(int x=0;x<iterations;x++)
 {
	  Stop();
	  motor(1, leftSide, leftSpeed);
	  motor(2, leftSide, leftSpeed);
	  delay(1000);
	 Stop();
	  motor(4, rightSide, rightSpeed);
	  motor(3, rightSide, rightSpeed);
		delay(1000);
	}
	Stop();
}
